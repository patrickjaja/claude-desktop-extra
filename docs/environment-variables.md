...
| `CLAUDE_PROFILE_QUIET` | `1` | Suppress the hint the launcher prints when `CLAUDE_PROFILE` names a profile that was not set up with `--create-profile` |
...
| `CLAUDE_USE_XWAYLAND` | `1` | Force XWayland instead of native Wayland. Needs an X server, i.e. `DISPLAY` set (on niri: run `xwayland-satellite`); without `DISPLAY` the launcher stays on native Wayland and logs why |
| `CLAUDE_GPU_BACKEND` | `angle-gl` | Render through ANGLE's OpenGL backend (`--use-gl=angle --use-angle=gl`) instead of the native Wayland/GBM path. Keeps GPU acceleration; try it before `CLAUDE_DISABLE_GPU` when the GPU process crashes on start ([#180](https://github.com/patrickjaja/claude-desktop-extra/issues/180)) |
| `CLAUDE_DISABLE_SANDBOX` | `1` | Launch with `--no-sandbox`. Every package ships a working Chromium sandbox, so this is an escape hatch only. The AppImage always runs without it (a FUSE mount cannot carry the SUID bit `chrome-sandbox` needs) |
...
| `CLAUDE_ELECTRON` | path | Override the Electron binary. Electron auto-loads the `resources/app.asar` next to it. The Nix wrapper and the AppImage set it; named profiles are refreshed from it too |
| `CLAUDE_LAUNCHER` | command or path | What "Start at login" and named-profile entries run. The launcher exports its own path; a package wrapper that sets it (the Nix package sets `claude-desktop`) keeps it, so those entries go through the wrapper. Ignored when not runnable |
