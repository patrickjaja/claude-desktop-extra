...
Reference documentation for the feature flag system in Claude Desktop's Electron app, to aid patch maintenance. The architecture/prose below was first written against v1.8555.2; minified names drift every release, so the **version history table at the bottom is the authoritative record of renames** (current through v2.7032.0; since v1.26832.0 the minifier emits backtick string literals, so grep with quote-tolerant classes; since v1.32352.1 it also emits numeric object keys UNQUOTED regardless of magnitude - `2973881027:DO(...)` - so a quoted-literal-only sweep reports false "removed" flags; sweep `[,{(]\d{6,10}:` keys too). Always cross-check a specific name there before trusting it.

**Code-split caveat (v1.19367.0+):** the main bundle is split into `index.js` (stub) + content-hashed `index.chunk-*.js` files + `index.pre.js`. From v1.26832.0 to v1.40609.0 there was a **second chunk family**, `index2.chunk-*.js`, belonging to the same logical bundle; **v1.46388.2 folds it back into one family** (158 `index.chunk-*` files, no `index2.chunk-*`; 155 in v1.49585.0, 199 in v2.2553.0). The orchestrator stages `index*.chunk-*` and therefore covers both layouts. Minified names are **per-chunk**: the same function can appear under different local names in different chunks (e.g. the boolean flag reader is `AI()` in the chunk that holds the registry but `t.Qu()` behind an import alias elsewhere). When grepping, search across ALL `index*.js` files (or the orchestrator's concatenation), and do not assume one canonical name per function.

Which chunk holds what also moves between releases - the static registry sat in `index2.chunk-*` in v1.28929.0 and in `index.chunk-*` since v1.30096.1 (registry, merger, GrowthBook store, reader exports AND the features Zod schema all in `index.chunk-BHnyA0sz.js` in v2.2553.0, `index.chunk-EgDgo76G.js` in v1.49585.0, `index.chunk-DrnJEXHK.js` in v1.46388.2, `index.chunk-Ci77nY41.js` in v1.40609.0, `index.chunk-DkKx6nb2.js` in v1.37937.0, `index.chunk-RiH_RK8u.js` in v1.34493.1, `index.chunk-CdfeLm1B.js` in v1.32885.1, `index.chunk-BFWQf1ai.js` in v1.32352.1). **An audit concat built from `index.chunk-*` alone is not the bundle**; it will show real, still-present flags as "removed". Always concatenate both families. Concatenate `index.pre.js` too: it is the managed-config/zod module and carries the deployment-key schema, and it is where the platform-gate audit finds counts the chunk-only concat misses. For **flag IDs** specifically it adds nothing - in v2.2553.0 the `index.pre.js`+`index.js`+chunks concat and the chunks-only concat yield the identical 415-ID set (354 in v1.49585.0), and none of the registry / merger / store-setter / production-gate anchors appears in `index.pre.js` at all. Validate the scope the same way every time: the OLD concat must reproduce the flag count this file records for the previous version before the NEW one is trusted.

## Overview

Feature flags are controlled by a 3-layer system (current minified names — see version-history table for the rename trail):

1. **`TV()` (static, module-scoped)** - Calls individual feature functions, builds base object (was `Jz()` in v1.49585.0, `OB()` in v1.46388.2, `iB()` in v1.40609.0, `fB()` in v1.37937.0, `uR()` in v1.34493.1, `RH()` in v1.32885.1, `JB()` in v1.32352.1, `$V()` in v1.30096.1, `X()` in v1.26832.0-v1.28929.0, `Ku()` in v1.20186.1-v1.21459.0, `yh()` in v1.19367.0, `sM()` in v1.18286.x). The maturity post-processor is called INLINE (since v1.32885.1) - `function TV(){let e=nBn();return ZBn({launch:gV,coworkBrowser:CBn(),artifactsPane:wV(),nativeQuickEntry:wzn(),...})}` in v2.2553.0 (was `function Jz(){let e=MEn();return bDn({launch:Hz,...})}` in v1.49585.0) - `launch` is a STATIC always-supported entry (flag `2976814254` is gone from the bundle), not an async override; the yukonSilver/Gems/GemsCache status is hoisted once (`let e=nBn()`, was `MEn()`/`cSn()`/`usn()`/`f3t()`/`tZt()`/`V2t()`/`e9t()`). The post-processor `ZBn()` (was `bDn()`/`USn()`/`Hsn()`/`U3t()`/`CZt()`/`l4t()`/`S9t()`/`u$t()`) stamps `maturity:"beta"` onto supported features in a list (beta list const `OLn`, was `Ewn`/`fbn`/`Ian`/`f2t`; in v2.2553.0 it still holds the single entry `surfaceTogglesPreview`; cosmetic, no gating). Supported constant `gV={status:"supported"}` (was `Hz`/`CB`/`tB`/`lB`/`cR`/`LH`/`qB`). Since v1.46388.2 the minifier emits plain double-quoted strings again (`{status:"unavailable"}`, not backticks), so quote-tolerant classes stay necessary across releases. 89 static entries in v2.2553.0 (73 in v1.49585.0, 71 in v1.46388.2, 64 in v1.40609.0, 62 in v1.37937.0, 59 in v1.34493.1, 57 in v1.32885.1).
2. **`QBn(e)` (async merger)** (was `xDn(e)` in v1.49585.0, `WSn(e)` in v1.46388.2, `oB(e)` in v1.40609.0, `mB(e)` in v1.37937.0, `wZt` in v1.34493.1, `u4t` in v1.32885.1, `C9t` in v1.32352.1) - since v1.37937.0 it takes an options argument (`e?.primeManagedPreviewPolicy`, which drives two extra non-destructured `Promise.all` slots). Spreads `TV()` and applies the async overrides: returns `{...TV(),...v}` where `v={telescopeBridge,louderPenguin,coworkKappa,coworkArtifacts,coworkAutoModeAlwaysAllowOverride,sideSessions,coworkWatchRecord,coworkCopperHeron,coworkWatchers,ccdGitEngine,sessionPrOwnership,violinBow,violinBowHomeSettings,computerUseAppScoped,wslForkSession,ccdConnectorTools,chillingSlothSshPuttyKeyConversion}` (**17 override keys as of v2.2553.0** over 16 distinct `Promise.all` slots; 13 in v1.46388.2-v1.49585.0, 10 in v1.34493.1, 9 in v1.32352.1-v1.32885.1, 8 in v1.26832.0-v1.30096.1. Four keys joined in v2.2553.0: `telescopeBridge`, `coworkCopperHeron`, `ccdConnectorTools`, `chillingSlothSshPuttyKeyConversion`. `sessionPrOwnership` still reuses the SAME destructured slot as `ccdGitEngine` (`ccdGitEngine:l,sessionPrOwnership:l`), i.e. both are gated by flag `959099749` via the 5s-delayed probe helper `DV(RB)` in v2.2553.0 (was `Yz(bz)`/`kB(nB)`/`aB(Bz)`/`pB(Jz)`/`dR(qL)`/`zH(SH)`/`YB(jB)`)). **Helpers (v2.2553.0 names):** `DV(e)` = `await eu(5e3); await rx(); e()?supported:unavailable` (5s delay + `waitForGrowthBookSettled`, was `Yz`/`kB`); `NBn(e)` = yukonSilver-gated `DV` (was `sDn`/`jSn`); `bBn(e)` = `Bz()?DV(e):yBn(e)`, where `Bz()` is the watch-record platform set `new Set(["darwin","win32"])`; `qBn(key,fn)` registers a gate in the change-observer map consumed by `XBn`. GrowthBook-read overrides: coworkKappa `123929380`, coworkArtifacts `2940196192`, coworkAutoModeAlwaysAllowOverride `4200321681` (all three via the yukonSilver-gated `NBn(()=>fx(id))`), sideSessions `2371478310` (`DV(()=>qBn("sideSessions",()=>fx("2371478310")))`), ccdGitEngine + sessionPrOwnership `959099749` (`DV(RB)`), wslForkSession `3724674924` (`DV(YLn)`; static entry `{status:"unavailable"}`, Windows-only feature so inert on Linux even when the flag is ON), **telescopeBridge `2864556627`** (`DV(LB)`, new in v2.2553.0), **ccdConnectorTools `4036648294`** (`DV(ILn)` = not-3p AND `px("4036648294",!0)`, new in v2.2553.0), **coworkCopperHeron `2661075028`** (`bBn(Xz)` where `Xz(){return Yz()?fx("2661075028"):!1}` - it needs watch-record enabled first, and watch-record is darwin/win32 only, so **inert on Linux**, new in v2.2553.0), **chillingSlothSshPuttyKeyConversion** (`IBn()` = `DV(()=>convertsPuttyKeys()&&selectSshTransport()==="openssh")`, no flag, new in v2.2553.0); `coworkWatchers` reads the scheduled-tasks module's `watchersEnabled()` (a config callback, not a direct flag read). **`violinBow` is `DV(vRn)` where `function vRn(){return!1}`** (was `Yz(Xwn)`/`kB(Ibn)`/`aB(Han)`/`pB(J2t)`/`dR(UYt)`) and **`violinBowHomeSettings` is `PBn(){return{status:"unavailable"}}`** (was `cDn()`/`MSn()`) - both hardcoded off, no flag, so they resolve `{status:"unavailable"}` on every platform (a `ViolinBowDisabledError` class exists for sessions that request it). **`computerUseAppScoped`** resolves through `VBn()` -> `RBn()` -> `qz()` (was `pDn()` -> `uDn()`), which is macOS 15+ only, so it is **unsupported on Linux**; `VBn()` waits 5s + GrowthBook settled + up to 3s for a network fetch via `onGrowthBookNetworkFetch` when online, and records the resolved status in a Set consumed by the change observer `XBn()`. **`epitaxyMcpApps` is a plain static `gV`** (always supported; flag `3516166472` is gone from the bundle) and **`chillingSlothSshWorktreeLocation` is static-only `jzn()` (was `oEn()`/`Ixn()`)** - supported unless the managed `workspace.allowedFolders` list is set (`disabled_by_enterprise`); flag `1315974108` is gone. `markTaskComplete` was removed in v1.17282.0; `launch` left the merger for the static registry (observed v1.26832.0). One destructured slot is dead in v2.2553.0: `f` takes `FBn()` (`{status:"unavailable"}`) and is never read into `v`. Three further `Promise.all` slots (`vV()` = overlay/virtualization probes (was `Wz()`/`TB()`), and the two `primeManagedPreviewPolicy` probes) are awaited but not destructured.
3. **IPC handler** - Calls merger, validates against schema, sends to renderer

**Reader identity is greppable, don't guess it.** The GrowthBook chunk (`index.chunk-BHnyA0sz.js` in v2.2553.0, `index.chunk-EgDgo76G.js` in v1.49585.0, `index.chunk-DrnJEXHK.js` in v1.46388.2, `index.chunk-Ci77nY41.js` in v1.40609.0, `index.chunk-DkKx6nb2.js` in v1.37937.0, `index.chunk-RiH_RK8u.js` in v1.34493.1, `index.chunk-CdfeLm1B.js` in v1.32885.1, `index.chunk-BFWQf1ai.js` in v1.32352.1, `index.chunk-BzNP_oYx.js` in v1.30096.1) ends its module with an export map that pairs the readable API name to that release's minified symbol:

```javascript
// v2.2553.0 (index.chunk-BHnyA0sz.js)
e.r({areFeaturesLoaded:()=>tx,getFeatureValue:()=>dx,getParsedFeatureValueForKey:()=>vx,
     growthbookEmitter:()=>Lb,initGrowthBook:()=>_Tt,isFeatureEnabled:()=>fx,
     isFeatureEnabledWithDefault:()=>px,isGrowthBookFreshForCurrentAccount:()=>mx,
     isGrowthBookFreshWithin:()=>hx,observeFeatureGate:()=>ox,onFeatureChange:()=>ax,
     onGrowthBookFreshChange:()=>gx,onGrowthBookNetworkFetch:()=>_x,
     peekFeatureGate:()=>sx,refreshGrowthBook:()=>ix,waitForGrowthBookReady:()=>nx,
     waitForGrowthBookSettled:()=>rx,withGateRefresh:()=>yTt})
// v1.49585.0 (index.chunk-EgDgo76G.js)
e.r({areFeaturesLoaded:()=>myt,getFeatureValue:()=>qx,getParsedFeatureValueForKey:()=>Qx,
     growthbookEmitter:()=>Dx,initGrowthBook:()=>fyt,isFeatureEnabled:()=>Jx,
     isFeatureEnabledWithDefault:()=>Yx,isGrowthBookFreshForCurrentAccount:()=>Xx,
     isGrowthBookFreshWithin:()=>wyt,observeFeatureGate:()=>Gx,onFeatureChange:()=>Wx,
     onGrowthBookFreshChange:()=>Zx,onGrowthBookNetworkFetch:()=>Tyt,
     peekFeatureGate:()=>gyt,refreshGrowthBook:()=>Ux,waitForGrowthBookReady:()=>Vx,
     waitForGrowthBookSettled:()=>Hx,withGateRefresh:()=>hyt})
...
Grep `isFeatureEnabled:\(\)=>` first; everything else follows from that block. New exports in v1.32352.1: **`isFeatureEnabledWithDefault`** (`$ct(id,dflt)` - boolean read with an explicit fallback, e.g. `$ct(\`3018088575\`,!0)`), `isGrowthBookFreshForCurrentAccount`, `onGrowthBookFreshChange`. New exports in v1.37937.0: **`peekFeatureGate`** (`gyt` in v1.49585.0, `rvt` in v1.46388.2), **`onGrowthBookNetworkFetch`** (`Tyt` / `uvt` - the one-shot network-fetch listener the tray/poll config uses, and since v1.49585.0 also the Cowork local-tasks kill-switch latch) and **`withGateRefresh`** (`hyt` / `nvt`). No export added or removed in v1.40609.0, v1.46388.2, v1.49585.0 or v2.2553.0 (17 exports). **Reader shapes** - bare in the GrowthBook chunk, module-qualified elsewhere (the import alias varies per chunk):

| Role | Bare (v2.2553.0) | Dotted (v2.2553.0) | v1.49585.0 (bare/dotted) | v1.46388.2 (bare/dotted) | v1.40609.0 (bare/dotted) | v1.37937.0 (bare/dotted) | v1.34493.1 (bare/dotted) | v1.32885.1 (bare/dotted) | v1.32352.1 (bare/dotted) | v1.30096.1 (bare/dotted) |
|------|------------------|---------------------|--------------------------|--------------------------|--------------------------|--------------------------|--------------------------|--------------------------|--------------------------|--------------------------|
| boolean | `fx(id)` | `.bI(id)` | `Jx` / `.pk` | `wS` / `.aD` | `HS` / `.yC` | `Xw` / `.Gx` | `XS` / `.Sb` | `Iw` / `.Ey` | `KE` / `.ty` | `AI` / `.Qu` |
| boolean with default | `px(id,dflt)` | `.xI(id,dflt)` | `Yx` / `.mk` | `TS` / `.oD` | `US` / `.bC` | `Zw` / `.Kx` | `Yit` / `.Cb` | `Uit` / - | `$ct` / - | - (new export) |
| value with default | `dx(id,dflt)` | `._I(id,dflt)` | `qx` / `.uk` | `CS` / `.nD` | `VS` / `.gC` | `Yw` / `.Hx` | `YS` / `.yb` | `Fw` / `.Cy` | `GE` / `.Qv` | `kI` / `.Yu` |
| keyed config value | `vx(id,key,dflt,zodType)` | `.vI(id,key,dflt,zodType)` | `Qx` / `.dk` | `OS` / `.rD` | `KS` / `._C` | `nT` / `.Ux` | `eC` / `.bb` | `zw` / `.wy` | `XE` / `.$v` | `NI` / `.Xu` |
| change listener | `ax(id,cb)` | `.TI(id,cb)` | `Wx` / `._k` | `vS` / `.lD` | `IS` / `.CC` | `Hw` / `.Yx` | `WS` / `.Eb` | `Aw` / `.ky` | `RE` / `.iy` | `wI` / `.td` |
| observe gate | `ox(id,cb)` | `.wI(id,cb)` | `Gx` / `.gk` | `yS` / `.cD` | `LS` / `.SC` | `Uw` / `.Jx` | `GS` / `.Tb` | - | - | - |

(In v2.2553.0: `areFeaturesLoaded` = `tx` / `.gI`, `isGrowthBookFreshForCurrentAccount` = `mx` / `.SI`, `waitForGrowthBookSettled` = `rx` / `.kI`, `onGrowthBookFreshChange` = `gx` / `.EI`, `refreshGrowthBook` = `ix` / `.DI`, `waitForGrowthBookReady` = `nx` / `.OI`, `withGateRefresh` = `yTt` / `.AI`, `growthbookEmitter` = `Lb` / `.Ft`, `isGrowthBookFreshWithin` = `hx` / `.CI`; `peekFeatureGate` (`sx`), `onGrowthBookNetworkFetch` (`_x`) and `initGrowthBook` (`_Tt`) have no dotted re-export. In v1.49585.0: `areFeaturesLoaded` = `myt` / `.lk`, `isGrowthBookFreshForCurrentAccount` = `Xx` / `.hk`, `waitForGrowthBookSettled` = `Hx` / `.xk`, `onGrowthBookFreshChange` = `Zx` / `.vk`, `refreshGrowthBook` = `Ux` / `.yk`, `waitForGrowthBookReady` = `Vx` / `.bk`, `withGateRefresh` = `hyt` / `.Sk`; `peekFeatureGate` (`gyt`), `onGrowthBookNetworkFetch` (`Tyt`) and `isGrowthBookFreshWithin` (`wyt`) have no dotted re-export. In v1.46388.2: `areFeaturesLoaded` = `mS` / `.tD`, `isGrowthBookFreshForCurrentAccount` = `ES` / `.sD`, `waitForGrowthBookSettled` = `gS` / `.pD`, `onGrowthBookFreshChange` = `DS` / `.uD`, `refreshGrowthBook` = `_S` / `.dD`, `waitForGrowthBookReady` = `hS` / `.fD`, `withGateRefresh` = `nvt` / `.mD`; `peekFeatureGate` (`rvt`), `onGrowthBookNetworkFetch` (`uvt`) and `isGrowthBookFreshWithin` (`lvt`) have no dotted re-export. In v1.37937.0: `observeFeatureGate` = bare `Uw` / dotted `.Jx`, `areFeaturesLoaded` = `Lw` / `.Vx`, `isGrowthBookFreshForCurrentAccount` = `Qw` / `.qx`, `waitForGrowthBookSettled` = `zw` / `.Qx`, `onGrowthBookFreshChange` = `eT` / `.Xx`, `refreshGrowthBook` = `Bw` / `.Zx`, `waitForGrowthBookReady` = `Rw` with **no dotted re-export** (nor do `isGrowthBookFreshWithin`, `onGrowthBookNetworkFetch`, `peekFeatureGate` and `withGateRefresh`). In v1.34493.1 these were `GS`/`.Tb`, `BS`/`.vb`, `ZS`/`.wb`, `HS`/`.kb`, `$S`/`.Db`, `US`/`.Ob`. The dotted names are `Object.defineProperty(exports,"<name>",...)` re-exports on the same chunk - grep `exports,"XX",{enumerable:!0,get:function(){return <bare>}` to rebuild the map. Older columns live in the version-history table.)
...
## The Production Gate `mV()` (was `Vz()` in v1.49585.0, `SB()` in v1.46388.2, `eB()` in v1.40609.0, `cB()` in v1.37937.0, `sR()` in v1.34493.1, `IH()` in v1.32885.1, `KB()` in v1.32352.1, `YV()` in v1.30096.1, `LM()` through v1.28929.0, `gM()` in v1.15962.x, `HR()` in v1.15200.0, historically `PM()`/`Nb()`/`DT()`/`MW()`)

In v2.2553.0: `function mV(e){return a.app.isPackaged?{status:"unavailable"}:e()}` (electron var `a`, unchanged shape).
In v1.49585.0: `function Vz(e){return o.app.isPackaged?{status:"unavailable"}:e()}` (electron var `o`).
...
**Features gated by the production gate (v2.2553.0):** `plushRaccoon`, `quietPenguin`, `wakeScheduler`, `framebufferPreview`, `rubberDuck` (new in v1.40609.0, `xBn(){return mV(()=>({status:"unavailable"}))}` in v2.2553.0 - dev-gated AND hardcoded unavailable) and `surfaceTogglesPreview`. **`iosSimulator`, `iosSimulatorH264`, `iosSimulatorAccessibilityReader` and `androidEmulator` left the gate in v2.2553.0**: the three iOS entries share `EBn()`, which returns a plain `{status:"unsupported",reason:"iOS Simulator panel requires macOS",unsupportedCode:"unsupported_platform"}`, and `androidEmulator` is `DBn()` = the `1118714395` device-companion flag with no production gate. **`bootstrapConfig` left the gate in v1.37937.0** - it is a plain always-supported entry (`bootstrapConfig:gV`), and `surfaceTogglesPreview` took its place (`mV((()=>gV))`)
...
### Layer 1: Static Registry (`TV()` in v2.2553.0, was `Jz()` in v1.49585.0, `OB()` in v1.46388.2, `iB()` in v1.40609.0, `fB()` in v1.37937.0, `uR()` in v1.34493.1; the sketch below uses the older `Np()`/`PM()` names)
...
### Layer 2: Async Merger (`QBn(e)` in v2.2553.0; was `xDn(e)` in v1.49585.0, `WSn(e)` in v1.46388.2, `oB(e)` in v1.40609.0, `mB(e)` in v1.37937.0, `wZt` in v1.34493.1, `u4t` in v1.32885.1, `C9t` in v1.32352.1, `nH` in v1.30096.1, `We` in v1.26832.0-v1.28929.0, `Yue`, `HSA` in v1.15962.x, `UcA` in v1.8089.1, `woA` in v1.7196.0)
...
#### New Flags in v2.7032.0

33 IDs appear that were absent from v2.2553.1; **all 33 are store-consulted** and templated in `js/growthbook_overrides.js`. 27 are read call-shaped, 6 through a hoisted const (`1234686386`, `1709366921`, `1800160441`, `3042392253`, `3469616823`, `3656976882`). Four further new IDs sit only in the 3p/self-hosted defaults map `_It` (`1417478306`, `2344289156`, `2479960717`, `3716359568`) and are not templated. The deployment-mode remote-allowed-keys set (`gIt`, was `vPt`) grows 26 -> 34 IDs, all of them also store-consulted. Reader shapes: bare `Ox`/`kx`/`Dx`/`Px`/`xx`/`Sx`/`Cx` in the GrowthBook chunk (`index.chunk-CxyX_nIQ.js`), dotted `.ez`/`.tz`/`.ZR`/`.QR`/`.az`/`.iz` elsewhere.

| Flag ID | Type | Purpose | Patched? |
|---------|------|---------|----------|
| `299664339` | boolean w/ default ON (`.tz`) | Conversation asides - passes the inline asides plugin options to sessions | No |
| `478602036` | boolean (`.ez`) | `unattendedBuiltinToolTrim` in the fork-session flag snapshot | No |
| `568988794` | boolean w/ default ON (`.tz`) | Local-session turn delegation (`turnIsDelegated`) | No |
| `575160718` | boolean w/ default OFF (`.tz`), inverted | Installed-plugins watcher semantic-digest reload kill switch | No |
| `725990316` | boolean w/ default ON (`kx`) | Adds `pluginsScope` to the minted credential; replaces `1689023552` | No |
| `922243720` | boolean w/ default ON (`.tz`) | Verified-cwd realpath check for AWF read-only tools | No |
| `1234686386` | boolean (const) | Device-companion sub-gate; inert, its parent check is hardcoded `false` | No |
| `1240761070` | boolean (`.ez`) | SSH failure-remedy kill switch (drops `review_host_key` and similar remedies) | No |
| `1439993118` | boolean w/ default ON (`kx`) | OAuth v2: clear the cached identity and retry after `refresh_rejected` | No |
| `1533587495` | boolean (`.ez`) | Pending-permission re-notify below the context-token threshold | No |
| `1593661558` | boolean w/ default OFF (`.tz`), inverted | CCD commands/agents list SWR + install-query wait kill switch | No |
| `1709366921` | boolean (const, `isFeatureEnabled`) | EXDEV copy gate for cross-device rename swaps | No |
| `1800160441` | boolean (const) | Skills org-block latch, sibling of `3656976882` | No |
| `1991650828` | boolean (`e.ez`) | Shell agent-socket kill switch (`isShellAgentSocketKillswitchOn`) | No |
| `2018643766` | boolean (`.ez`), inverted | git source-ref fetch memo / `--porcelain` fetch kill switch | No |
| `2405551282` | boolean w/ default ON (`.tz`) | Stamped-proof check mode: `enforce` vs `observe` | No |
| `2521146974` | boolean (`.ez`) | git status-cache warm-up mode: `revert` vs `prime` | No |
| `2675972342` | boolean w/ default ON (`.tz`) | Auto-mode classifier stand-down for force-ask tools | No |
| `2716316020` | boolean (`.ez`) | Worktree tombstone lane kill switch | No |
| `2766542666` | boolean (`e.ez`), inverted | Base-branch sync before a PR autofix, kill switch | No |
| `3042392253` | boolean w/ default ON (const, `.tz`) | `ccdBaseBranchSync` host tool (merge base into the session branch) | No |
| `3059213726` | boolean (`.ez`) | Session title suggestions (`offerSessionTitle`, persisted `titleOffers`) | No |
| `3126429675` | boolean w/ default OFF (`.tz`/`.ZR`), inverted | Remote-plugin enabled-state SWR / app-owned roots kill switch | No |
| `3162918178` | boolean w/ default ON (`.tz`) | CCD folder setup tools (`request_directory`/`change_directory`/`propose_branch`) | No |
| `3216202605` | boolean w/ default ON (`e.tz`) | Sleep auto-resume offer, with `4044603026` | No |
| `3469616823` | boolean (const) | Skill-creation org-block override (skips the `blocked_by_org_*` lockout) | No |
| `3484205914` | boolean w/ default ON (`.tz`) | Built-in tool availability gate, sibling of `2300874989`/`1486104550` | No |
| `3656976882` | boolean (const) | Skills availability gate (per-account `blocked_by_org_admin` tracking) | No |
| `3664723602` | boolean (`.ez`) | Code-session `BROWSER` shim over the `ccd-br` socket; honours `authentication.disableDeepLinks` | No |
| `3778508830` | boolean (`.ez`) | Parallel git checkout workers: half the cores (2..8) instead of 2..16 | No |
| `4018447017` | boolean (`.ez`) | Skill folder mount/upload path gate | No |
| `4082309721` | boolean (`.ez`) | git worktree warm-up sub-gate, sibling of `3000061192`/`630659907` | No |
| `4267385443` | boolean (`.ez`), inverted | git `hooksPath`/`worktreeConfig` memo kill switch | No |

**Removed in v2.7032.0:** 5 IDs are gone, all templated: `1689023552` (credential `pluginsScope`, superseded by `725990316`), `1904135164` (plugin download `moveExistingAside`), `2309422447` (`mergeMessageBufferIfActive`), `2800354941` (3p defaults-map entry for deterministic list sorting) and `2941281426` (CLI file checkpointing). One never-templated defaults-map ID also left (`3762988844`).

Template 359 -> 387 (`CATALOG_EXPECTED` bumped): +33, -5.

**Feature (registry/schema) changes in v2.7032.0:** **added 6**, removed 0. Static registry `TV()` -> `wH()`, 89 -> 95 entries; Zod schema `e8e` -> `$5e` (validator `Eg(e)` = `R5e.safeParse`, was `pg`/`z6e`), 90 -> 96 keys, still a non-strict `.partial()` object co-located with the registry in `index.chunk-CxyX_nIQ.js`; the extra schema key is still `louderPenguin`. None of the new entries is platform-gated:

| Feature | Registry value | Notes |
|---------|----------------|-------|
| `keepAwake` | `PV()?hH:{status:"unavailable"}` | `PV()` = `workspace.keepAwakeEnabled!==false` (the new `keepAwakeEnabled` deployment key) |
| `conversationFolderGrants` | `hH` | Always supported |
| `inlinePastes` | `hH` | Always supported |
| `sideSessionOfferCloudStart` | `hH` | Always supported |
| `mcpScheduledTaskAlwaysAllow` | `W().type==="3p"&&!yKn()?{status:"unavailable"}:hH` | 3P-conditional |
| `mcpScheduledTaskApprovalLifetime` | `W().type==="3p"&&xKn()?hH:{status:"unavailable"}` | 3P-only (the new `mcpScheduledTaskApprovalLifetimeDays` key) |

The merger `QBn(e)` -> `qYn(e)` keeps the same 17 override keys over 16 destructured slots (`sessionPrOwnership` still shares `ccdGitEngine`'s slot, flag `959099749` via `EH(IV)`; slot `f` = `kYn()` is still dead). Helpers: 5s-delay probe `DV` -> `EH`, yukon+delay `NBn` -> `DYn`, production gate `mV()` -> `pH()` (same shape, electron var `a`), supported const `gV` -> `hH`, yukonSilver hoist `nBn()` -> `YJn()`, maturity post-processor `ZBn()` -> `KYn()` (beta list `OLn` -> `hKn`, still just `surfaceTogglesPreview`). The production gate still wraps exactly `plushRaccoon`, `quietPenguin`, `wakeScheduler` and `surfaceTogglesPreview` (plus the hardcoded-unavailable `rubberDuck` getter `hYn()`). `chillingSlothFeat` (`SJn()`) and `chillingSlothLocal` (`TJn()`) still return supported, `ccdPlugins` is `hH`, `computerUse` is `fYn()` (darwin/win32 set, Linux unsupported), `louderPenguin` is `dYn()` = darwin/win32 + flag `4116586025`.

**enable_local_agent_mode in v2.7032.0:** all remaining anchors verified present with exactly one match across the staged bundle - Patch 1's darwin/win32 quietPenguin inner (`lYn()`, consumed as `quietPenguin:pH(lYn)`), Patch 1b's `case"darwin":case"linux":return"unix"` mapper, Patch 3's merger pattern (only `return{...wH(),...v}}` matches and `wH()` is registry-verified: `quietPenguin:` sits 345 chars into its body), Patch 4's `quietPenguinEnabled:!1,louderPenguinEnabled:!1`, and the `["anthropic-client-os-platform",r.default.platform]` header guard. All 6 Patch 3 override keys (`quietPenguin`, `louderPenguin`, `chillingSlothFeat`, `chillingSlothLocal`, `ccdPlugins`, `computerUse`) are in the 96-key Zod schema, and all but `louderPenguin` are in the registry. **Patch 3n was retired in this bump**: `resolveSshControllerForMcp` became the module-level function `My(e){if(e)return m.r(e)}`, still gate-free, and 3n only asserted that absence, so it is removed per Rule 4/6 (`EXPECTED_PATCHES` is now 6). CU chicago family intact (`2486083521`, `1291166712`, `1346958739`, `40173473`, plus the reconcile listener `36693946`); buddy `2358734848` present. **add_growthbook_overrides** sub-patch B still matches exactly once, on the features-store setter `jDt(e,t){ix=t;let n=rx;rx=EDt(e);...}` (was `cTt`/`Vb`/`Hb`/`rTt`), and `EDt(e)` is still the deployment-mode `hardcodedMainGrowthBookFeatures()` + `growthBookRemoteAllowedKeys()` filter (hardcoded map `hIt`, allowed-keys set `gIt`).

**Deployment-key schema in v2.7032.0:** 178 `flatKey`s (in `index.pre.js` and in `index.chunk-CxyX_nIQ.js`), up from 171 - 8 added, 1 removed. `js/extra_settings_main.js` carries all 178. Added: `inferenceIdpOidc` + `inferenceIdpAuthFlow` (a new `external-idp` value of `inferenceCredentialKind` - the user's own OIDC token sent to the gateway or Bedrock proxy; the Entra broker flow is documented as not supported on Linux), `keepAwakeEnabled`, `claudeCodeProcessWrapperEnabled` (start the Code tab's Claude Code through the Claude Code managed-settings `processWrapper` launcher; macOS and Linux only), `codeAllowedRepositories`, `deviceToolsEnabled` (self-hosted only), `disableLocalConfigCache` and `mcpScheduledTaskApprovalLifetimeDays`. Removed: `selfHostedOidcTokenStorage`. Eight keys dropped the `@next` marker and now carry `availableInVersion:"2.2553.0"` (`allowedPluginMcpServers`, the four `builtinBrowser*` keys, `importThirdPartyHistoryIntoOrgUUID`, `inferenceBedrockRegion`, `inferenceCredentialHelperWindows`).

#### New Flags in v2.2553.0

78 IDs appear that were absent from v1.49585.0; **69 are store-consulted** (templated in `js/growthbook_overrides.js`), 8 are 3p-defaults-map-only (`1002481325`, `1119418671`, `2477702615`, `2774291247`, `2831057674`, `3088953445`, `3762988844`, `3809766863` - unquoted keys in `yPt`/`bPt`/`uPt`/`pPt`) and 1 (`3310072118`) sits only in the deployment-mode remote-allowed-keys set `vPt`. Reader shapes: bare `fx`/`px`/`dx`/`vx`/`ax`/`ox`/`sx` in the GrowthBook chunk (`index.chunk-BHnyA0sz.js`), dotted `.bI`/`.xI`/`._I`/`.vI`/`.TI`/`.wI` elsewhere.

**Sweep the dot-separated allowed-keys string too.** `vPt=new Set("3602629573.3436441689....1130670564".split("."))` packs 26 flag IDs into ONE quoted literal, so a `["\`](\d{6,10})["\`]` sweep misses every ID that lives only there and reports it as removed. `4055864154` and `3310072118` are exactly that case.

| Flag ID | Type | Purpose | Patched? |
|---------|------|---------|----------|
| `21606909` | boolean w/ default ON (`px`) | App-menu **Alt+letter mnemonics** - hidden `Alt+<initial>` accelerators appended to each top-level submenu open that menu | No |
| `126748301` | boolean w/ default ON (`.xI`) | Built-in CLI tool-name list gate, read next to the `PreToolUse` hook check | No |
| `270847253` | boolean w/ default ON (`.xI`) | Code-tab session-management MCP tool group - OFF marks that group unavailable | No |
| `275501886` | listener (`.TI`) + latch | Sentry DWOC drop kill-switch - latches the `sentryDwocDropDisabledLatched` preference and is observed live; also a 3p defaults-map key | No |
| `323247490` | boolean w/ default ON (`.xI`, hoisted `Le`) | `[git]` `core.hooksPath` discovery via `git config -z --list --name-only` | No |
| `341781248` | boolean w/ default ON (`.xI`) | Side-sessions event injection sibling gate, with `3025587613` / `3531612483` | No |
| `386883921` | value w/ default ON (`._I`) | Plugin sync module gate, third of the `3183093548` / `2294160313` trio | No |
| `500434831` | boolean (`.bI`) | Worktree pool fast-acquire kill-switch | No |
| `539375764` | boolean w/ default ON (`.xI`) | PR author-association module gate, with `846970509` / `599480072` | No |
| `599480072` | boolean w/ default ON (`.xI`) | PR/CI module sibling of `539375764` | No |
| `630659907` | boolean (`.bI`) | Worktree overlay-gate kill-switch - ON stops waiting for the overlay before checkout | No |
| `666373776` | boolean (`.bI`) | SSH host autostart on non-gesture calls - ON drops `withholdAutostart` for warm-up calls | No |
| `846970509` | boolean (`.bI`) | PR/CI module boolean sibling of `539375764` / `599480072` | No |
| `1130670564` | boolean (`fx`) | In-app-browser log persistence for non-Anthropic pages - ON (like 3p) stops persisting them; also in `vPt` | No |
| `1240280160` | boolean (`.bI`) | Worktree checkout-mode sibling gate, read with the `1754160972` mode config | No |
| `1308449919` | boolean (`.bI`) + observed (`.wI`) | Session activity-ledger collection kill-switch, live | No |
| `1341990989` | boolean w/ default ON (`.xI`) | Code-tab MCP tool-set gate, sibling of `270847253` / `2015900644` / `3556866454`; also in `vPt` | No |
| `1486104550` | boolean w/ default ON (`.xI`) | Remote-control session MCP verb gate, sibling of `2300874989`; also in `vPt` | No |
| `1496761016` | boolean w/ default ON (`.xI`) | **Terminal panel tools** - OFF answers "The Terminal panel tools are turned off for this app or organization; use Bash if it is available."; also in `vPt` | No |
| `1517059896` | boolean w/ default ON (`.xI`) | CLI version-floor gate, third of the `96832454` / `2153038254` family | No |
| `1552109632` | boolean (`.bI`) | Worktree entry-claims kill-switch | No |
| `1606273333` | boolean w/ default ON (`.xI`) | Scheduled-task MCP tool gate | No |
| `1689023552` | boolean w/ default ON (`.xI`) | CCD credential **plugins scope** - adds `pluginsScope` to the minted token | No |
| `1699596383` | boolean (`fx`) | Scheduled-task "unreachable" result classification; also in `vPt` | No |
| `1847129881` | boolean (`.bI`, hoisted `vt`) | Device Cowork **artifact list** tool - ON announces the newer list-artifacts tool instead of the `1947305033` one | No |
| `2001396010` | boolean (`.bI`, hoisted `Va`) | `[remote-file]` outputs commit kill-switch - ON refuses staged file commits with `disabled` | No |
| `2015900644` | boolean w/ default ON (`.xI`) | Code-tab MCP tool-set gate, sibling of `270847253` / `1341990989` / `3556866454`; also in `vPt` | No |
| `2057990550` | boolean w/ default ON (`.xI`, hoisted `Jf`) | **`ccd_settings` MCP tools** - `get_settings` / `set_setting` over the Code tab preferences; also in `vPt` | No |
| `2071783254` | boolean (`.bI`) | `isNetworkWaitKillswitchOn` - its own single-export module | No |
| `2088149064` | value (`dx` via the `zRn` wrapper, hoisted `KRn`) | Device-companion platform sub-gate, same wrapper as `1118714395`; inert - its `tV()` prefix is hardcoded `false` | No |
| `2161994580` | boolean w/ default ON (`.xI`) | **`ccd_view` MCP tools** - `show_pane` / `close_pane` / `get_layout`; also in `vPt` | No |
| `2240013170` | boolean (`.bI`) | Automatic session title check after a turn | No |
| `2278171137` | boolean (`.bI`) | Disclaimed-cwd TCC preflight kill-switch | No |
| `2280348969` | boolean (`.bI`) | CLI process-group reap gate | No |
| `2300874989` | boolean w/ default ON (`.xI`) | Remote-control session MCP verb gate, sibling of `1486104550`; also in `vPt` | No |
| `2303338333` | boolean w/ default ON (`.xI`, behind `areFeaturesLoaded`) + observed (`.wI`) | Archived-session hints in list results | No |
| `2375401243` | boolean w/ default ON (`.xI`) | Shell-quoting heuristic for host-to-VM path rewriting | No |
| `2487484738` | boolean (`.bI`) | Sets `CLAUDE_CODE_ENABLE_BG_SHELL_PRESSURE_REAP=1` in the local-agent CLI env | No |
| `2661075028` | boolean (`fx`) | **`coworkCopperHeron`** async merger override - gated behind watch-record being enabled, which is darwin/win32 only, so **inert on Linux** | No |
| `2683764500` | boolean (`.bI`) | Wait for the CLI process to exit during session teardown | No |
| `2696933583` | boolean w/ default ON (`.xI`, hoisted `ko`) | **ccd editor MCP tools** - `open_in_editor` / `reveal_path` / `request_keep_awake`; also in `vPt` | No |
| `2753512555` | boolean w/ default ON (`.xI`) | Teleport-to-cloud handoff tool; also requires claude.ai product features | No |
| `2787764107` | keyed config value (`vx`, hoisted `C7`) | Held-routine-release device-reconnect config - `enabled` / `minIntervalSeconds` / `minGapSeconds` | No |
| `2825088777` | boolean (`.bI`, hoisted `d`) | `[remote-cu]` remote computer-use control - longer resolution-cache TTL and lock lease when ON | No |
| `2862654137` | boolean (`.bI`) | SSH remote worktree trust-key refresh, sibling of `581786799` | No |
| `2917981554` | boolean (`.bI`, hoisted `Tt`) | UNC path allowance when opening a connected folder in the Cowork VM; also a 3p defaults-map key | No |
| `2948506237` | boolean (`fx`) | Auth callback nonce mismatch tolerance - ON accepts a callback whose nonce does not match | No |
| `2961678273` | boolean (`.bI`) | `[logRetention]` transcript-retention sweep kill-switch - ON reports `skipped:"disabled"` | No |
| `3000061192` | boolean (`.bI`) | Legacy shadow-git scan, read with `4286026368` / `630659907` | No |
| `3093754864` | boolean w/ default ON (`.xI`) | Session auto-archiving decision module gate | No |
| `3223040764` | boolean w/ default ON (`.xI`) | **claude-in-chrome MCP tool set** for Cowork VM sessions | No |
| `3419852220` | boolean (`.bI`) | Relaunch auto-resume switch-off - ON reports `switched_off` and resumes nothing after a relaunch | No |
| `3512519265` | boolean (`.bI`) | CLI force-end kill-switch on update - ON leaves still-running CLIs alone | No |
| `3523350481` | boolean (`.bI`) | Worktree index-heal kill-switch | No |
| `3556866454` | boolean w/ default ON (`.xI`) | Code-tab MCP tool-set gate, sibling of `270847253` / `1341990989` / `2015900644`; also in `vPt` | No |
| `3608536666` | value (`dx` via the `zRn` wrapper, hoisted `WRn`) | Device-companion platform sub-gate, same wrapper as `1118714395`; inert - its `tV()` prefix is hardcoded `false` | No |
| `3649943861` | boolean (`fx`) | `claude://` **artifact deep links** - opens `/artifact/<id>` routes from the custom scheme | No |
| `3721849379` | boolean w/ default ON (`.xI`) | CCD OAuth token renew-ahead margin | No |
| `3791538283` | boolean (`fx`, behind `isGrowthBookFreshForCurrentAccount`) | Space memory keyless-device path | No |
| `3832912485` | boolean w/ default ON (`px`, hoisted `DUn`) | `[browser-live-preview]` socket discovery | No |
| `3958333548` | boolean w/ default ON (`px`) | In-app browser reuse for a path that is not a live session | No |
| `3977864451` | boolean w/ default ON (`px`) | Agent-emulated viewport reset at turn end | No |
| `4036648294` | boolean w/ default ON (`px`) | **`ccdConnectorTools`** async merger override - `W().type!=="3p"&&px("4036648294",!0)` | No |
| `4092838515` | boolean w/ default ON (`.xI`, hoisted `ma`) | **Storage MCP tools** - `get_storage_usage` and the 30/60/90-day inactive-worktree cleanup; also in `vPt` | No |
| `4100354330` | boolean (`.bI`) | Session link-classification kill-switch | No |
| `4236214729` | boolean (`.bI`) | Scratch-session folder offer on first spawn | No |
| `4255448834` | boolean (`fx`) | Code **project** deep link - ON hands `claude://` project links to the fallback route | No |
| `4279816466` | boolean w/ default ON (`px`) | Media permission requests carry `securityOrigin` / `mediaTypes` into the permission check | No |
| `4286026368` | boolean (`.bI`) | Shadow-git fetch-refspec check kill-switch | No |

**Removed in v2.2553.0:** 17 IDs are gone. Nine were store-consulted and templated: `629684104`, `748063099`, `751369921`, `1323782925`, `2216901299`, `2339084909`, `2720310975`, `3646818354`, `3783846612`. Eight were never templated (defaults-map- or allowed-keys-only): `232278890`, `693564285`, `890748467`, `954625922`, `3070110303`, `3150971238`, `4083972287`, `4272200640`.

Template 299 -> 359 (`CATALOG_EXPECTED` bumped): +69, -9.

**Feature (registry/schema) changes in v2.2553.0:** **added 16**, removed 0. Static registry 73 -> 89 entries; Zod schema (`e8e`, validator `m8e`; was `C1e`/`N$e`) 74 -> 90 keys, still a non-strict `.partial()` object co-located with the registry in `index.chunk-BHnyA0sz.js`. The extra schema key is `louderPenguin`, which lives only in the merger. New entries:

| Feature | Registry value | Notes |
|---------|----------------|-------|
| `telescopeBridge` | `{status:"unavailable"}` static | Async merger override `DV(LB)` = flag `2864556627` |
| `ccdConnectorTools` | `{status:"unavailable"}` static | Async merger override `DV(ILn)` = not-3p AND flag `4036648294` |
| `chillingSlothSshPuttyKeyConversion` | `{status:"unavailable"}` static | Async merger override `IBn()` = `convertsPuttyKeys()` AND `selectSshTransport()==="openssh"`; no flag |
| `coworkCopperHeron` | `yBn(Xz)` (darwin/win32 gate) | Async merger override `bBn(Xz)`; needs watch-record enabled + flag `2661075028`. **Unsupported on Linux**, same platform set as `coworkWatchRecord` |
| `ccdLandingTools` | `qBn("ccdLandingTools",Azn)?gV:{status:"unavailable"}` | `Azn()` = local-session policy readable and not disabled, AND `chillingSlothFeat` supported |
| `ccdMcpVerbsMidTurn` | `gV` | Always supported |
| `ccdPluginSourceRouting` | `gV` | Always supported |
| `openWithConsentAnyLanding` | `gV` | Always supported |
| `seededKind` | `gV` | Always supported |
| `sendNowEscalation` | `gV` | Always supported |
| `sideSessionProposalPicks` | `gV` | Always supported |
| `turnDiscardedEvent` | `gV` | Always supported |
| `desktopHome` | `QOe(K())===void 0?{status:"unavailable"}:gV` | Driven by the managed `desktopHome` deployment key |
| `selfHostedSessions` | `uC(xV)?gV:{status:"unavailable"}` | Driven by the new `selfHostedUrl` managed-config block |
| `selfHostedCowork` | `SV()?gV:{status:"unavailable"}` | Same block |
| `iosSimulatorAccessibilityReader` | `EBn()` | macOS-only `unsupported_platform`, shares `iosSimulator`'s getter |

Gate reshuffles: `chillingSlothFeat` (`kzn()`) and `chillingSlothLocal` (`Mzn()`) still return `{status:"supported"}` unconditionally; `computerUse` is still `vBn()` -> `zz()` = `Rz.has(process.platform)` over `new Set(["darwin","win32"])`; `louderPenguin` is still `_Bn()` = darwin/win32 + flag `4116586025`. The iOS/Android simulator entries left the production gate (see above). **Linux-facing registry entries worth knowing** (both present since v1.49585.0, unchanged): `quickEntryGlobalShortcut` = `Tzn()`, which reports `unsupported_session` with "Global keyboard shortcuts require the GlobalShortcuts desktop portal, which this Wayland session does not provide"; and `popoutTitleBarOverlay` = `wBn()`, which reports `unsupported_session` "Pop-out title bar overlay is not available under Wayland" whenever `--ozone-platform=wayland` or a Wayland session is detected. The beta-maturity list `OLn` still holds the single entry `surfaceTogglesPreview`.

**enable_local_agent_mode in v2.2553.0:** all anchors verified present with exactly one match across the staged bundle - Patch 1's darwin/win32 gate resolves to the quietPenguin inner `function hBn(){return process.platform!=="darwin"&&process.platform!=="win32"?{status:"unavailable"}:{status:"supported"}}` (registry consumes it as `quietPenguin:mV(hBn)`, dev-build-only as before), Patch 1b's `case"darwin":case"linux":return"unix"` mapper (`kLn`) + `FB.files[kLn("linux")][e]` index both present once, Patch 3's merger pattern matches only `return{...TV(),...v}}` and `TV()` is registry-verified, Patch 3n's `resolveSshControllerForMcp(e){if(e)return m.r(e)}` is still gate-free (flag `1496676413` absent from the bundle), Patch 4's `quietPenguinEnabled:!1,louderPenguinEnabled:!1` matches once, and the `["anthropic-client-os-platform",r.default.platform]` header guard is present. All 6 Patch 3 override keys (`quietPenguin`, `louderPenguin`, `chillingSlothFeat`, `chillingSlothLocal`, `ccdPlugins`, `computerUse`) are in the 90-key Zod schema. CU chicago family intact (`2486083521`, `1291166712`, `1346958739`, `40173473`); buddy `2358734848` present. No patch edit needed. **add_growthbook_overrides** sub-patch B still lands on the features-store setter `cTt(e,t)` and wraps the transform call `rTt(e)`, which is still the deployment-mode hardcoded-features filter.

**Deployment-key schema in v2.2553.0:** 171 `flatKey`s in `index.pre.js`, up from 143 in v1.49585.0 - 28 added, none removed. `js/extra_settings_main.js` carries all 171. The additions: a self-hosted execution block (`selfHostedUrl`, `selfHostedCredentialKind`, `selfHostedOidc`, `selfHostedOidcAuthFlow`, `selfHostedOidcTokenStorage`, `selfHostedToken`, `selfHostedCredentialHelper`, `selfHostedCredentialHelperTtlSec`, `selfHostedCredentialHelperTimeoutSec`, `selfHostedCredentialHelperSilentRefreshEnabled` - all `visibility:"internal-only"`, `executionTarget:"selfHosted"`); session retention (`chatSessionRetentionDays`, `coworkSessionRetentionDays`, `codeSessionRetentionDays`, `sessionRetentionHold`); model controls (`defaultModelEffort`, `alwaysStartWithDefaultModel`, `modelCatalogEnabled`, `modelCatalogUrl`); credential-helper variants (`inferenceCredentialHelperArgs`, `inferenceCredentialHelperWindows`); `inferenceFoundryBaseUrl`; `allowedPluginMcpServers`; `coworkVmIpv6Enabled`; `desktopHome`; `scheduledTasksEnabled`; `sshTransport`; `dangerousMaxVersion`; and the 1p-scoped `importThirdPartyHistoryIntoOrgUUID`.

#### New Flags in v1.49585.0

8 IDs appear that were absent from v1.46388.2; **all 8 are store-consulted** (templated in `js/growthbook_overrides.js`). No new 3p-defaults-map-only or remote-allowed-keys-only IDs. Reader shapes: bare `Jx`/`Yx`/`qx`/`Qx`/`Wx`/`Gx`/`gyt` in the GrowthBook chunk (`index.chunk-EgDgo76G.js`), dotted `.pk`/`.mk`/`.uk`/`.dk`/`._k`/`.gk` elsewhere.

| Flag ID | Type | Purpose | Patched? |
|---------|------|---------|----------|
| `30891128` | boolean (`Jx`, hoisted `Dti`, re-exported as `.Gt`) | rc-serve **tool hosts offered** gate - `toolHostsOffered` / `wantsGitEntry` in the remote-control serve device module; quartet with `1183214304`/`3764441751`/`1410651677` | No |
| `946844604` | boolean (`.pk`) | Cowork memory remote sync v2 - `Li()` = flag AND `canSyncCoworkMemoryRemotely()` AND not HIPAA-restricted; the sync entry point answers `refusal:"flag_off"` when OFF | No |
| `1550954491` | boolean w/ default ON (`Yx`, hoisted `Bri`) | Bare **Alt-key tap opens the main menu popup** (`before-input-event` state machine on the main and design webContents, `Dmi?.("keyboard")`); modifier combos and auto-repeat pass through | No |
| `1986048577` | value (`.uk(id,72e5)`, hoisted `Zt`) | `resolveRoutineSheetLateWindowMs` - routine (scheduled task) sheet late window, default 2h, clamped to 60s..24h, `<=0` disables | No |
| `2595385640` | boolean (`.pk`) | Session-storage inventory telemetry - `folderWalkAllowed()`, sibling of the `1028094019` emitter gate | No |
| `3076393065` | boolean (`.pk`, hoisted `ss`) | **`ccd_window` MCP tools** - `open_session_in`, `close_split`, `set_sidebar_collapsed`, `get_window_layout` (split-view layout of Code sessions; layout only, nothing is sent to the other session) | No |
| `3576697941` | listener (`Wx` via the latch helper `xen`) | **`storeReadCache` boot latch** - a flip writes the `storeReadCacheLatched` preference, read back at boot by `kz()` next to `earlyWindowShow`/`bootPlaceholder`; sibling of the `2685067074` `earlyWindowShowLatched` latch | No |
| `3634338308` | peek (`gyt(id)?.value===!0`) on every network fetch (`Tyt`) | **Cowork local-tasks kill-switch** - `HEn()` latches the `coworkLocalTasksOffLatched` preference; while latched (and not 3p) the yukonSilver chain `IEn()` returns `{status:"unsupported",reason:"Your organization's Cowork settings don't allow tasks to run on this computer.",unsupportedCode:"disabled_by_org_policy"}`. Un-latches only when the flag reads false AND the account is signed in with fresh GrowthBook data (`VEn()`). Templated as **DO NOT ENABLE** | No |

**Removed in v1.49585.0:** `2079084046`, `2129861473`, `3635490728` - all three were 3p-defaults-map-only (unquoted keys in the deployment-mode hardcoded flag map), never store-consulted, never templated. **No store-consulted flag disappeared**; every one of the 291 v1.46388.2 template IDs is still read in v1.49585.0.

Template 291 -> 299 (`CATALOG_EXPECTED` bumped): +8, -0.

**Feature (registry/schema) changes in v1.49585.0:** **added 2** - `coworkScheduledTaskRepoint` (static `Hz`, always supported) and `localSessionsWithoutGit` (static `Hz`, always supported). **Removed 0.** Static registry 71 -> 73 entries; Zod schema (`C1e`, symbol `sh`, was `N$e`/`xh`) 72 -> 74 keys, still a non-strict `.partial()` object and now co-located with the registry in `index.chunk-EgDgo76G.js`. Merger override object unchanged at 13 keys with the same composition. Gate reshuffles: none; `chillingSlothFeat` (`aEn()`) and `chillingSlothLocal` (`sEn()`) still return `{status:"supported"}` unconditionally, `computerUse` is still `KEn()` -> `AR()` = `lyn.has(process.platform)` over `new Set(["darwin","win32"])`, `louderPenguin` is still `GEn()` = darwin/win32 + flag `4116586025`. The only behavioural additions are the Cowork local-tasks kill-switch (`3634338308`, above) and the new `computerUseAppScoped` probe timing (`pDn()`, see Overview item 2). The beta-maturity list `Ewn` still holds the single entry `surfaceTogglesPreview`.

**enable_local_agent_mode in v1.49585.0:** all anchors verified present with exactly one match across the staged bundle - Patch 1's darwin/win32 gate resolves to the quietPenguin inner `function UEn(){return process.platform!=="darwin"&&process.platform!=="win32"?{status:"unavailable"}:{status:"supported"}}` (registry consumes it as `quietPenguin:Vz(UEn)`, dev-build-only as before), Patch 1b's `case"darwin":case"linux":return"unix"` mapper (`Dwn`) + `.files[Dwn("linux")][e]` index both present once, Patch 3's merger pattern matches only `return{...Jz(),...m}}`, Patch 3n's `resolveSshControllerForMcp` is still gate-free (flag `1496676413` absent from the bundle), Patch 4's `quietPenguinEnabled:!1,louderPenguinEnabled:!1` matches once, and the `["anthropic-client-os-platform",...platform]` header guard is present. All 6 Patch 3 override keys are in the 74-key Zod schema. CU chicago family intact (`2486083521`, `1291166712`, `1346958739`, `40173473`); buddy `2358734848` present. No patch edit needed. **add_growthbook_overrides** sub-patch B still lands on the features-store setter `nyt(e,t)` and wraps the transform call `Zvt(e)`, which is still the deployment-mode hardcoded-features filter (`hardcodedMainGrowthBookFeatures()` + `growthBookRemoteAllowedKeys()`).

**Deployment-key schema in v1.49585.0:** unchanged - 143 `flatKey`s in `index.pre.js` (and the same 143 duplicated in `index.chunk-EgDgo76G.js`), identical set to v1.46388.2; `js/extra_settings_main.js` catalog stays at 143 (version stamp bumped). The 3p-dir resolver (`lW()`: `CLAUDE_USER_DATA_DIR` pins userData, else the `-3p` sibling) and the mode decision (`WG(e)`: chooser-disabled / persisted `deploymentMode` `"1p"` escape hatch / hybrid-pointer / persisted) are byte-for-byte the same logic as v1.46388.2. The `inferenceProvider` schema node gained a `values:{mantle:{availableInVersion:"1.13576.0"}}` support annotation; `mantle` (Bedrock Mantle) has been in the provider Set (`["anthropic","gateway","vertex","bedrock","mantle","foundry"]`) since before v1.46388.2 but is not offered by the panel's enum options, which list only `gateway`/`vertex`/`bedrock`/`foundry`/`anthropic`.
...
| v2.7032.0 | `wH()` (chunk `index.chunk-CxyX_nIQ.js`) | `qYn(e)`, anchor `return{...wH(),...v}}` | `pH()` | **Bump v2.2553.1 -> v2.7032.0 (re-minify; 199 -> 214 `index.chunk-*` files, single family, double-quoted literals).** Registry `TV()` -> `wH()`, yukonSilver hoist `nBn()` -> `YJn()`, maturity post-processor `ZBn()` -> `KYn()` (beta list `OLn` -> `hKn`, still just `surfaceTogglesPreview`), supported const `gV` -> `hH`, production gate `mV()` -> `pH()`, 5s-delay helper `DV` -> `EH`, yukon+delay helper `NBn` -> `DYn`, async merger `QBn(e)` -> `qYn(e)` (same 17 override keys). Static registry 89 -> 95 (+6: `keepAwake`, `conversationFolderGrants`, `inlinePastes`, `sideSessionOfferCloudStart`, `mcpScheduledTaskAlwaysAllow`, `mcpScheduledTaskApprovalLifetime`; none platform-gated); Zod schema `e8e` -> `$5e`, 90 -> 96 keys. GrowthBook chunk `index.chunk-BHnyA0sz.js` -> `index.chunk-CxyX_nIQ.js`; readers `fx`/`.bI` -> `Ox`/`.ez` (boolean), `px`/`.xI` -> `kx`/`.tz`, `dx`/`._I` -> `Dx`/`.ZR`, `vx`/`.vI` -> `Px`/`.QR`, `ax`/`.TI` -> `xx`/`.az`, `ox`/`.wI` -> `Sx`/`.iz`, `sx` -> `Cx` (peek), `_x` -> `Nx` (network-fetch listener); features-store setter `cTt` -> `jDt` (store `Vb` -> `rx`, tag `Hb` -> `ix`, transform `rTt` -> `EDt`). Flags: +33 store-consulted, +4 defaults-map-only, -5 templated, -1 never-templated; template 359 -> 387. Deployment-key schema 171 -> 178 (+8, -1), catalog updated. enable_local_agent_mode: Patch 3n retired (assert-only, `resolveSshControllerForMcp` is now the gate-free module function `My(e)`), `EXPECTED_PATCHES` 7 -> 6; all other anchors match once. |
| v2.2553.0 | `TV()` (chunk `index.chunk-BHnyA0sz.js`) | `QBn(e)`, anchor `return{...TV(),...v}}` | `mV()` | **Bump v1.49585.0 -> v2.2553.0 (re-minify + the 2.x line; 155 -> 199 `index.chunk-*` files, single family, double-quoted literals).** Registry `Jz()` -> `TV()`, yukonSilver hoist `MEn()` -> `nBn()`, maturity post-processor `bDn()` -> `ZBn()` (beta list `Ewn` -> `OLn`, still just `surfaceTogglesPreview`), supported const `Hz` -> `gV`, production gate `Vz()` -> `mV()`, 5s-delay helper `Yz` -> `DV`, yukon+delay helper `sDn` -> `NBn`, async merger `xDn(e)` -> `QBn(e)`. Merger override object 13 -> 17 keys: +`telescopeBridge` (`2864556627`), +`coworkCopperHeron` (`2661075028`, behind watch-record so inert on Linux), +`ccdConnectorTools` (`4036648294`, not-3p), +`chillingSlothSshPuttyKeyConversion` (no flag); one destructured slot (`f` = `FBn()`) is now dead. Static registry 73 -> 89 (+16, -0); Zod schema `C1e` -> `e8e`, 74 -> 90 keys. iOS/Android simulator entries left the production gate. GrowthBook chunk `index.chunk-EgDgo76G.js` -> `index.chunk-BHnyA0sz.js`; readers `Jx`/`.pk` -> `fx`/`.bI` (boolean), `Yx`/`.mk` -> `px`/`.xI`, `qx`/`.uk` -> `dx`/`._I`, `Qx`/`.dk` -> `vx`/`.vI`, `Wx`/`._k` -> `ax`/`.TI`, `Gx`/`.gk` -> `ox`/`.wI`, `gyt` -> `sx` (peek), `Tyt` -> `_x` (network-fetch listener); features-store setter `nyt` -> `cTt` (store `jx` -> `Vb`, tag `Mx` -> `Hb`, transform `Zvt` -> `rTt`). Flags: +69 store-consulted, +8 defaults-map-only, +1 allowed-keys-only, -9 templated, -8 never-templated; template 299 -> 359. Deployment-key schema 143 -> 171 (+28, -0), catalog updated. enable_local_agent_mode: all 7 anchors match once, no edit. |
| v1.49585.0 | `Jz()` (chunk `index.chunk-EgDgo76G.js`) | `xDn(e)`, anchor `return{...Jz(),...m}}` | `Vz()` | **Bump v1.46388.2 -> v1.49585.0 (re-minify; 158 -> 155 `index.chunk-*` files, single family, double-quoted literals).** Registry `OB()` -> `Jz()`, yukonSilver hoist `cSn()` -> `MEn()`, maturity post-processor `USn()` -> `bDn()` (beta list `fbn` -> `Ewn`, still just `surfaceTogglesPreview`), supported const `CB` -> `Hz`, production gate `SB()` -> `Vz()`, 5s-delay helper `kB` -> `Yz`, yukon+delay helper `jSn` -> `sDn`, async merger `WSn(e)` -> `xDn(e)`. Merger override object unchanged (13 keys, same composition); `computerUseAppScoped` slot now `pDn()` (waits for a network fetch up to 3s before probing `uDn()`, still macOS 15+ only). Static registry 71 -> 73 (+`coworkScheduledTaskRepoint`, +`localSessionsWithoutGit`, both static `Hz`); Zod schema `N$e` -> `C1e` 72 -> 74 keys, now in the registry chunk. GrowthBook chunk `index.chunk-DrnJEXHK.js` -> `index.chunk-EgDgo76G.js`; readers `wS`/`.aD` -> `Jx`/`.pk` (boolean), `TS`/`.oD` -> `Yx`/`.mk`, `CS`/`.nD` -> `qx`/`.uk`, `OS`/`.rD` -> `Qx`/`.dk`, `vS`/`.lD` -> `Wx`/`._k`, `yS`/`.cD` -> `Gx`/`.gk`, `rvt` -> `gyt` (peek), `uvt` -> `Tyt` (network-fetch listener); features-store setter `G_t` -> `nyt` (store `eS` -> `jx`, tag `tS` -> `Mx`, transform `B_t` -> `Zvt`). Flags: +8 store-consulted (`30891128`, `946844604`, `1550954491`, `1986048577`, `2595385640`, `3076393065`, `3576697941`, `3634338308` - the last is a server kill-switch that latches Cowork local tasks OFF), -3 defaults-map-only (`2079084046`, `2129861473`, `3635490728`); template 291 -> 299. Deployment-key schema unchanged at 143. enable_local_agent_mode: all 7 anchors match once, no edit. |
