...
> **Roster re-verified against v2.7032.0** (2026-09-23; bump v2.2553.1 -> v2.7032.0, v2.2553.1 reproduces every v2.2553.0 count below): registration mechanism unchanged - `registerInternalMcpServer` still present with exactly one registration, exported as `registerInternalMcpServer:()=>Oxe` in `index.chunk-CxyX_nIQ.js`. The server-definition shape (`createToolHandler` / `handleToolCall` / `getDynamicTools`) and the derived reserved-name list are unchanged.
>
> **Server-uuid map: 29 -> 30 entries.** Added `ccd_turn:"9d2c4a71-6b3e-4f58-a1c9-7e5b0d8f3a26"`; nothing removed, no uuid changed. **One new backend server, no platform check, available on Linux:** `ccd_turn` (#30) with two `alwaysLoad` tools, `update_plan` (shows the turn's step plan; `{items:[...]}` for the whole plan, `{advance:true}` or `{done:[n],start:m}` to move it; max 12 steps, statuses `pending` / `in_progress` / `completed`) and `wrap_up` (fills the "next-step box" under the closing message: a short recap, one question and 1-3 answer buttons such as `commit` / `create_pr` / `merge` / `fix_ci` / `address_comments` / `resolve_conflicts` / `verify` / `archive` / `new_session`). Definition: ``{serverName:"ccd_turn",tools:[{name:"update_plan",...,alwaysLoad:!0},{name:"wrap_up",...,alwaysLoad:!0}],isEnabled:e=>e.sessionType==="ccd"&&e.turnBoxEnabled===!0,handleToolCall:...}``, where `turnBoxEnabled` is the per-session `turnBox` option, and the CCD allowed-tools list adds its two tool ids only when `ccd_turn` is active. **Three existing servers gained a tool:** `ccd_host` + `sync_with_base_branch`, `ccd_view` + `set_transcript_view`, `ccd_session_mgmt` + `set_session_output_style`. Nothing was removed or renamed.
>
> **Counting method (v2.7032.0)** - audit concat = `cat .vite/build/index.pre.js .vite/build/index.js .vite/build/index.chunk-*.js` (216 files, 214 chunks). `serverName:` -> **138** (136 on v2.2553.x); `mcp__<server>__<tool>` literals -> **90** (82), **58 unique** (55): the three new unique literals are `mcp__ccd_turn__update_plan`, `mcp__ccd_turn__wrap_up` and `mcp__ccd_session_mgmt__list_sessions` (the latter a new system-prompt mention of an existing tool). Bundled 3p servers byte-identical: `office365-mcp-stdio.mjs` 7,215,327 B, `pdfExtractorProcess.mjs` 425,295 B, `pdf.worker.mjs` 1,089,813 B, `github-mcp/github-mcp-server` 19,931,320 B. Packaged `app.asar` 39,371,700 -> 40,353,537 B; bundled Electron 44.2.0 -> 44.4.3.
>
...
| Field | Value |
|-------|-------|
| Server name | `"ccd_window"` (constant `ts` in v1.49585.0, `index.chunk-D4n72M0W.js`) |
| Gating | `isEnabled:e=>e.sessionType==="ccd"&&ss()` with `ss=()=>t.pk("3076393065")` (GrowthBook) - **no platform check**, works on Linux |
| Added in | v1.49585.0 |

Lets a Code session arrange the desktop's window layout - typically to show a session it just spawned with `ccd_session.start_session` next to itself. Tool ids are built as ``os=[`mcp__${ts}__${ns}`, ...]`` (template literals - invisible to the flat `mcp__` grep). The server talks to the renderer over the new eIPC service **`WindowLayout`** (`setLayoutCommandListening`, `reportLayoutCommandResult`; events `command`, `layoutCommand`): the main process dispatches a `layoutCommand` with a 256-char-capped payload and a deadline, and the renderer answers via `reportLayoutCommandResult`. Commands are refused with a textual reason when the main window is gone (`no_window`), the renderer is not listening (`not_listening` - still loading, signed out, or on a page without the layout host), or the reply times out. Every call is recorded as the `desktop_ccd_window_tool` telemetry event.

| Tool | Description |
|------|-------------|
| `open_session_in` | Show another session (`"self"` or one this session started) as `split` pane, pop-out `window`, or `focus` in the main pane; layout only, nothing is sent to that session |
...
Claude Desktop creates 4 additional MCP servers **dynamically per cowork/dispatch session** (5 for first-party cowork sessions since v1.49585.0, which also get the `memory` cloud-memory relay described below). These are NOT registered via `LYA()` - they are created inline in the session manager and passed to the Claude Code CLI via `sdkMcpServers` in `--mcp-config`.
...
### Cloud memory relay (`memory`) - cowork sessions, v1.49585.0+

Injected into the cowork session's `mcpServers` map right after the `cowork` server is built (`Xi(e,n,{sessionId,sessionType:"cowork",accountIds,getCurrentOrgId,memoryEnabled})` in `index.chunk-CSphapxR.js`, log prefix `[memoryMcpServer]`). It is an in-process `McpServer` named **`memory`** (`t.jE`, version `1.0.0`) whose `tools/list` and `tools/call` are forwarded as JSON-RPC to `POST <apiHost>/v2/ccr-sessions/-/memory/mcp` (headers `X-Session-Key`, `x-organization-uuid`, query `expected_account=<accountId>`; 5 s timeout; 4 MiB request cap). Only **`memory_read`** and **`memory_list`** are exposed (filter set `ji`); the write-side tools (`memory_write`, `memory_str_replace`, `memory_append`, `memory_delete`) stay with the VM-side implementation. The two relay tool ids are appended to the session's `allowedTools` and appear in the read-only tool allowlist as flat literals `mcp__memory__memory_read` / `mcp__memory__memory_list`.

Gate: `Li()` = GrowthBook `946844604` AND `canSyncCoworkMemoryRemotely()` (true only for the first-party provider; the 3p provider returns `!1`) AND not HIPAA (`!t.zS()`); additionally skipped when the session config has `memoryEnabled===!1`. Refusal reasons surface in the log as `not_first_party`, `hipaa_restricted`, `flag_off`, `no_account_or_org`, `account_mismatch`. A user-configured MCP server already named `memory` is replaced (warn-logged: "replacing a configured MCP server named memory with the cloud memory relay"). **No platform check** - works on Linux.
...
| Version | Changes |
|---------|---------|
| v1.49585.0 | Registration export `registerInternalMcpServer:()=>A_e` (`index.chunk-EgDgo76G.js`); 155 `index.chunk-*`. **New backend server `ccd_window`** (#25: 4 tools, flag `3076393065`, CCD-only, no platform check) with a new eIPC service `WindowLayout`; **new per-session SDK-type `memory` cloud-memory relay** for cowork sessions (`memory_read`/`memory_list` only, flag `946844604` + first-party + non-HIPAA). Enum array 19 -> 20 (`+ccd_window`), `serverName:` 119 -> 120, flat `mcp__` literals 49 -> 51 unique. M365 server renamed `office365-mcp.mjs` -> `office365-mcp-stdio.mjs` (7,322,713 -> 7,215,327 B, tool inventory unchanged). Electron 42.10.0 -> 44.2.0, agent SDK 0.3.260 -> 0.3.265. |
